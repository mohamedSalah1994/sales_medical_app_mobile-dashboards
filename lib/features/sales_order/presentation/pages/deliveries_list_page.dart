...
/// Lists ERP deliveries for the logged-in user (filtered by [salesEmployeeCode] from auth).
/// Cards match [SalesOrderListPage]; tap opens [DeliveryFromSalesOrderPage.viewExisting] with full detail.
/// Search opens a bar to load `GET /api/erp/deliveries/{docEntry}` and show the result on the delivery page.
class DeliveriesListPage extends StatefulWidget {
  const DeliveriesListPage({super.key, this.showScaffold = true});
...
class _DeliveriesListPageState extends State<DeliveriesListPage> {
  ...
  late final VoidCallback _deliveriesScrollListener = _onDeliveriesScroll;

  @override
  void initState() {
    super.initState();
    _scrollController.addListener(_deliveriesScrollListener);
    WidgetsBinding.instance.addPostFrameCallback((_) => _load());
  }

  @override
  void dispose() {
    _scrollController.removeListener(_deliveriesScrollListener);
    _scrollController.dispose();
    _docEntryController.dispose();
    super.dispose();
  }

  void _onDeliveriesScroll() {
    if (!mounted) return;
    ...
    if (!st.deliveriesListHasMore ||
        st.isLoadingMoreDeliveriesList ||
        st.isLoadingDeliveriesList) {
      return;
    }
    final sapCode =
        context
    ...
    cubit.loadDeliveriesList(salesEmployeeCode: sapCode, append: true);
  }
  ...
  void _load() {
    final sapCode =
        context
            ...
            ?.user
            .sapSalesEmployeeCode;
    context.read<SalesOrderCubit>().loadDeliveriesList(
  ...
  void _showDeliveriesFilterSheet() {
  ...
  void _openDelivery(SalesOrderListItemModel order) {
  ...
  /// Doc-entry search row (filters live in AppBar like Journey Plans).
  ...
  Widget build(BuildContext context) {
    ...
    final listBody = BlocBuilder<SalesOrderCubit, SalesOrderState>(
      buildWhen:
          (p, c) =>
              p.deliveriesList != c.deliveriesList ||
              p.isLoadingDeliveriesList != c.isLoadingDeliveriesList ||
              p.isLoadingMoreDeliveriesList != c.isLoadingMoreDeliveriesList ||
              p.deliveriesListHasMore != c.deliveriesListHasMore ||
              p.deliveriesListError != c.deliveriesListError,
      builder: (context, state) {
        if (state.isLoadingDeliveriesList && state.deliveriesList.isEmpty) {
          return const Center(child: CircularProgressIndicator());
        }
        if (state.deliveriesListError != null && state.deliveriesList.isEmpty) {
          return Center(
            child: Padding(
        ...
        final list = state.deliveriesList;
        final footer = state.deliveriesListHasMore ? 1 : 0;
      ...
      appBar: AppBar(
        ...
        actions: [
          BlocBuilder<SalesOrderCubit, SalesOrderState>(
            buildWhen:
                (p, c) =>
                    p.hasActiveDeliveriesFilters !=
                    c.hasActiveDeliveriesFilters,
            builder: (context, state) {
              return Stack(
                ...
                children: [
                  IconButton(
                    tooltip: 'Filters',
                    onPressed: _showDeliveriesFilterSheet,
                    icon: Icon(
                      Icons.filter_list,
                      color:
                          state.hasActiveDeliveriesFilters
                              ? AppColors.primary
                              : AppColors.textSecondary,
                    ),
                  ),
                  if (state.hasActiveDeliveriesFilters)
                    Positioned(
                      right: 8,
                      top: 8,
                      child: Container(
...
/// Delivery row: doc **entry**, customer; **status** top-right (Open/Close styling on pill only).
class _DeliveryListCard extends StatelessWidget {
  const _DeliveryListCard({required this.order, required this.onTap});
